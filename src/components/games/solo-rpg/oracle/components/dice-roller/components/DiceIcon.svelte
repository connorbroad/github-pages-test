<script lang="ts">
    /**
     * Reusable dice icon component
     */
    export let numSides: number;
    export let value: number | null = null;
    export let offset: { x: number; y: number; r: number } = {
        x: 0,
        y: 0,
        r: 0,
    };
...
<span
    class="relative inline-block h-16 w-16"
    style="transform: translate({offset.x}px, {offset.y}px) rotate({offset.r}deg); transition: {animated
        ? 'transform 0.1s'
        : 'none'};">
    {#if numSides === 4}
        <!-- D4: triangle -->
        <svg class="absolute left-0 top-0 z-0 h-16 w-16 opacity-90" viewBox="0 0 32 32" width="32" height="32" aria-hidden="true">
    ...
    {:else if numSides === 20}
        <!-- D20: icosahedron (simplified as polygon) -->
        <svg class="absolute left-0 top-0 z-0 h-16 w-16 opacity-90" viewBox="0 0 32 32" width="32" height="32" aria-hidden="true">
            <polygon
    ...
    {/if}
    <span class="pointer-events-none absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 text-xl font-bold text-blue-700">{value !== null ? value : numSides}</span>
</span>
