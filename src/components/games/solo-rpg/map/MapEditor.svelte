<script lang="ts">
	import { getTileMapById, loadMaps, saveMaps, type MapEntity, type MapObject, type TileRef } from "../data/storage-utils";
    ...
    function loadMap(){
		...
		if (map) {
			...
			for (const o of map.objects) {
				if (o.kind === 'tile' && o.tile) 
                    refByMapId.set(o.tile.tileMapId, true);
			}

			const missingTileMaps: string[] = [];
			for (const id of refByMapId.keys()) {
				if (!getTileMapById(id)) 
                    missingTileMaps.push(id);
			}
			if (missingTileMaps.length) {
				...
				map.objects = map.objects.filter(o => !(o.kind === 'tile' && o.tile && missingTileMaps.includes(o.tile.tileMapId)));
			}
		}
    }
