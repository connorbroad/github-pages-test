...
import {
    ...
    DungeonRoom,
    type DungeonSegment,
} from "../layout/entities/dungeon-segment.ts";
...
const possibleSegmentDoorLocations = [
    DoorLocation.North,
    DoorLocation.South,
    DoorLocation.East,
    DoorLocation.West,
...
export class DungeonLayoutGenerator {
    ...
    public static tryGetRandomCorridorConnectionSlotsBetweenRooms(
        room1ExistingDoors: DungeonDoor[],
        room2ExistingDoors: DungeonDoor[]
    ): DoorLocation[] {
        ...
        let shuffledUnusedDoorLocationsRoom1 = unusedDoorLocationsRoom1.sort(
            () => Math.random() - 0.5
        );
        let shuffledUnusedDoorLocationsRoom2 = unusedDoorLocationsRoom2.sort(
            () => Math.random() - 0.5
        );
    ...
    private static generateExtraCorridorConnections(floor: DungeonFloor, numRooms: number) {
        ...
        for (let i = 0; i < numExtraCorridors; i++) {
            ...
            let corridorConnection = this.tryGetRandomCorridorConnectionSlotsBetweenRooms(
                room1ExistingDoors,
                room2ExistingDoors
            );
    ...
    private static generateFloorCorridorsFromRooms(floor: DungeonFloor) {
        ...
        let roomsWithFreeDoors = floor.rooms.filter(
            (room) => floor.doors.filter((door) => door.Room == room).length < 4
        ); // rooms where there are less than 4 doors assigned to that room
    ...
    private static generateFloorRoomsFromCorridors(floor: DungeonFloor, numDesiredRooms: number) {
        ...
        let corridorsWithFreeDoors = floor.corridors.filter(
            (corridor) => floor.doors.filter((door) => door.Corridor == corridor).length < 2
        );
    ...
    private static getAllFreeDoorLocationsForRoom(
        existingDoorsInRoom: DungeonDoor[]
    ): DoorLocation[] {
        let takenDoorLocations = existingDoorsInRoom.map((door) => door.RoomDoorLocation);
        return possibleSegmentDoorLocations.filter(
            (doorLocation) => !takenDoorLocations.includes(doorLocation)
        );
    }

    private static getRandomFreeDoorLocationForCorridor(
        existingDoorsInCorridor: DungeonDoor[]
    ): DoorLocation {
        let takenDoorLocations = existingDoorsInCorridor.map((door) => door.CorridorDoorLocation);
        let availableDoorLocations = possibleSegmentDoorLocations.filter(
            (doorLocation) => !takenDoorLocations.includes(doorLocation)
        );
        return availableDoorLocations[Math.floor(Math.random() * availableDoorLocations.length)];
    }
    ...
    private static generateRoomsShape(): RoomShape {
        let basicShape =
            segmentBasicShapeChance[Math.floor(Math.random() * segmentBasicShapeChance.length)];
        if (basicShape == SegmentShape.Basic) return RoomShape.Basic;
    ...
    private static generateCorridorShape(): CorridorShape {
        let basicShape =
            segmentBasicShapeChance[Math.floor(Math.random() * segmentBasicShapeChance.length)];
        if (basicShape == SegmentShape.Basic) return CorridorShape.Basic;
        return corridorOtherShapeChance[
            Math.floor(Math.random() * corridorOtherShapeChance.length)
        ];
    }

    private static generateRandomDoorStatus(): DoorStatus {
        return doorStatusChance[Math.floor(Math.random() * doorStatusChance.length)];
    }
}
